...
public class SbPagingInfo {
	// 1페이지당 출력할 데이터를 끊어내기 위해 필요한 멤버들
		...
		// 한 번에 세팅하자
		public void setAttribute(int totalPostCnt, int pageNo, int viewPostCntPerPage) {
			setViewPostCntPerPage(viewPostCntPerPage);
		...
		public void setAttribute(int value, String kind, int viewPostCntPerPage) {
			setViewPostCntPerPage(viewPostCntPerPage);
			if (kind == "pageNo") {
